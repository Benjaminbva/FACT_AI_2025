...
class GPUTrainer:
    ...
    def train(self, iterations,
        ...
        for iteration in (bar := tqdm(
            ...
            # forward pass of GCN model -> gets classification probabilities
            cont_out = self.discriminator(cont_data.to(self.device), edge_weight=cont_data.edge_weight)
            disc_out = self.discriminator(disc_data.to(self.device), edge_weight=disc_data.edge_weight)

            # if targeted classes are predicted between min_p and max_p -> edit budget penatly weight
            # breaks training loop if expected number of edges is smaller than target_size
    ...
    @torch.no_grad()
    def quantitative(self, sample_size=500, sample_fn=None):
        sample_fn = sample_fn or (lambda: self.evaluate(bernoulli=True))
        p = []
        for i in range(sample_size):
            p.append(self.predict(sample_fn())["probs"][0].cpu().numpy().astype(float))
        return dict(label=list(self.dataset.GRAPH_CLS.values()),
                    mean=np.mean(p, axis=0),
                    std=np.std(p, axis=0))

    @torch.no_grad()
    def quantitative_baseline(self, **kwargs):
        return self.quantitative(sample_fn=lambda: nx.gnp_random_graph(n=self.sampler.n, p=1/self.sampler.n),
                                 **kwargs)

    # TODO: do not rely on dataset for drawing
    @torch.no_grad()
    def evaluate(self, *args, show=False, connected=False, return_probs=False, **kwargs):
        self.sampler.eval()
